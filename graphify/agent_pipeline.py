...
def _pipeline_excludes(scan_root: Path, output_root: Path) -> list[str]:
    """Exclude files generated by this pipeline when output lives under input."""
    excludes = ["/.csc/kb/state.json"]
    try:
        relative_output = output_root.relative_to(scan_root)
    except ValueError:
        return excludes
    if relative_output.parts:
        excludes.append("/" + relative_output.as_posix().rstrip("/") + "/")
    return excludes


def _root_marker_path(value: str) -> Path:
    """Accept native paths and Git-Bash /c/... paths on Windows."""
...
def prepare_agent_pipeline(
    ...
    *,
    ...
    extra_excludes = _pipeline_excludes(scan_root, output_root)
    ...
    if incremental:
        if not graph_path.is_file() or not manifest_path.is_file():
            raise AgentPipelineError(
                "--update requires an existing graph.json and manifest.json; run a full build first"
            )
        existing_public_graph = _read_json(graph_path, label="existing graph")
        existing_directed = bool(existing_public_graph.get("directed", False))
        if directed and not existing_directed:
            raise AgentPipelineError(
                "--directed does not match the existing undirected graph; "
                "run a full directed rebuild instead"
            )
        directed = existing_directed
        if root_marker.exists():
            recorded_root = _root_marker_path(root_marker.read_text(encoding="utf-8-sig"))
            if str(recorded_root).casefold() != str(scan_root).casefold():
                raise AgentPipelineError(
                    f"--update input root mismatch: graph was built from {recorded_root}, "
                    f"but this run received {scan_root}"
                )
        ...
        work_files = detection.get("new_files", {})
    else:
        detection = detect(scan_root, extra_excludes=extra_excludes)
        work_files = detection.get("files", {})
    root_marker.write_text(str(scan_root), encoding="utf-8")
    ...
    for raw_path in work_files.get("code", []):
        path = Path(raw_path)
    ...
    semantic_files = [
        ...
        for path in work_files.get(category, [])
    ]
    ...
    plan = {
        ...
        "phase": (
            "noop"
            if incremental
            and not detection.get("new_total", 0)
            and not detection.get("deleted_files", [])
            else "prepared"
        ),
        ...
        "changed_files": detection.get("new_files", {}) if incremental else {},
        "deleted_files": detection.get("deleted_files", []) if incremental else [],
        "chunks": chunks,
        "counts": {
            "total_files": detection.get("total_files", 0),
            "changed_files": detection.get("new_total", 0) if incremental else detection.get("total_files", 0),
            "deleted_files": len(detection.get("deleted_files", [])) if incremental else 0,
...
def _graph_to_extraction(graph: Any, *, input_tokens: int = 0, output_tokens: int = 0) -> dict[str, Any]:
    """Convert a merged NetworkX graph back to the extraction contract."""
    nodes = []
    for node_id, attributes in graph.nodes(data=True):
        node = dict(attributes)
        node.pop("community", None)
        node.pop("community_name", None)
        node.pop("norm_label", None)
        node["id"] = node_id
        nodes.append(node)
    ...
    for source, target, attributes in graph.edges(data=True):
        ...
        true_source = edge.pop("_src", source)
        true_target = edge.pop("_tgt", target)
        edge["source"] = true_source
...
def build_agent_pipeline(
    ...
    *,
    ...
    graph_path = graph_dir / "graph.json"
    if plan.get("incremental") and graph_path.exists():
        existing_graph = _read_json(graph_path, label="existing graph")
        existing_node_ids = {
            str(node.get("id")) for node in existing_graph.get("nodes", [])
            if isinstance(node, dict) and node.get("id") is not None
        }
    semantic_errors = validate_agent_extraction(
        semantic,
        external_node_ids=ast_node_ids | existing_node_ids,
        require_resolved_endpoints=True,
    )
    if semantic_errors:
        raise AgentPipelineError(
    ...
    if plan.get("incremental"):
        def _source_key(value: str) -> str:
            source_path = Path(value)
            if not source_path.is_absolute():
                source_path = scan_root / source_path
            return str(source_path.resolve())

        changed_files = [
            path
            for paths in plan.get("changed_files", {}).values()
            for path in paths
        ]
        contributed_sources = {
            _source_key(str(node.get("source_file")))
            for source in (ast, semantic)
            for node in source.get("nodes", [])
            if isinstance(node, dict) and node.get("source_file")
        }
        empty_changed_sources = [
            path for path in changed_files
            if _source_key(path) not in contributed_sources
        ]
        prune_sources = list(plan.get("deleted_files", [])) + empty_changed_sources
        # Semantic data is merged first so AST attributes win on duplicate IDs,
        # matching the full-build contract above.
        graph = build_merge(
            [semantic, ast],
            graph_path,
        ...
        extraction = _graph_to_extraction(
            graph,
            input_tokens=semantic["input_tokens"],
            output_tokens=semantic["output_tokens"],
        )
        semantic_output_node_ids = {
            str(node.get("id")) for node in extraction["nodes"]
            if isinstance(node.get("metadata"), dict)
            and node["metadata"].get("entity_type")
        }
        semantic_all = {
            "nodes": [
                node for node in extraction["nodes"]
                if str(node.get("id")) in semantic_output_node_ids
            ],
            "edges": [],
    ...
    _write_compatible_graph(
        ...
        force=force or bool(plan.get("incremental")),
    )
    ...
    result = {
        ...
        "changed_files": plan.get("counts", {}).get("changed_files", 0),
        "deleted_files": plan.get("counts", {}).get("deleted_files", 0),
    }
    plan["phase"] = "built"
